...
class PoisonedCIFAR10(CIFAR10):
    def __init__(self,
        ...
        super(PoisonedCIFAR10, self).__init__(
            ...
            benign_dataset.target_transform,
            download=False)
        self.data = self.data[np.array(self.targets)!=y_target]
        self.targets = np.array(self.targets)[np.array(self.targets)!=y_target]
        total_num = len(benign_dataset)
        poisoned_num = int(total_num * poisoned_rate)
        assert poisoned_num >= 0, 'poisoned_num should greater than or equal to zero.'
        tmp_list = list(range(total_num))
...
class PoisonedDataset(Dataset):
    def __init__(self,
        ...
        # Remove samples with the target label from the dataset
        self.data = self.data[self.targets != y_target]
        self.targets = self.targets[self.targets != y_target]

        total_num = len(self.data)
        poisoned_num = int(total_num * poisoned_rate)
        assert poisoned_num >= 0, 'poisoned_num should be greater than or equal to zero.'
        
        self.poisoned_set = frozenset(random.sample(range(total_num), poisoned_num))
        # Prepare transforms
        if benign_dataset.transform is None:
            self.poisoned_transform = Compose([])
        else:
            self.poisoned_transform = copy.deepcopy(benign_dataset.transform)
        self.poisoned_transform.transforms.insert(poisoned_transform_index, AddDatasetFolderTrigger(pattern, weight))
        if benign_dataset.target_transform is None:
            self.poisoned_target_transform = Compose([])
        else:
            self.poisoned_target_transform = copy.deepcopy(benign_dataset.target_transform)
        self.poisoned_target_transform.transforms.insert(poisoned_target_transform_index, ModifyTarget(y_target))
    ...
    def __getitem__(self, index):
        ...
        if index in self.poisoned_set:
            ...
            if self.benign_dataset.transform is not None:
                img = self.benign_dataset.transform(img)

            if self.benign_dataset.target_transform is not None:
                target = self.benign_dataset.target_transform(target)
...
def CreatePoisonedDataset(benign_dataset, y_target, poisoned_rate, pattern, weight, poisoned_transform_index, poisoned_target_transform_index):
    ...
    if class_name == DatasetFolder:
        ...
        return PoisonedDataset(benign_dataset, y_target, poisoned_rate, pattern, weight, poisoned_transform_index, poisoned_target_transform_index)
...
class LabelConsistent():
    ...
    def _get_adv_dataset(self, dataset, adv_model, adv_dataset_dir, adv_transform, eps, alpha, steps, y_target, poisoned_rate):
        ...
        target_adv_dataset = CustomDatasetFolder(
